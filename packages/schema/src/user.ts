...
export const username=z.string({message:"username must be string"}).trim().min(3,{message:"username must have at least 3 characters"}).max(20,{message:"username must have at most 20 characters"}).regex(/^[a-zA-Z0-9]*$/,{message:"username must have only alphanumeric characters"});
export const email=z.string({message:"email must be string"}).trim().email({message:"email must be a valid email"});
export const mobile=z.string({message:"mobile must be string"}).trim().regex(/^\d{10}$/, {message:"mobile must be a valid phone number"});
...
export const mobileObj = z.object({
    mobile: mobile
}).strip();
